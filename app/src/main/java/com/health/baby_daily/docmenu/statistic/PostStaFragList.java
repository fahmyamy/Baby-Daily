...
import com.health.baby_daily.adapter.PostListAdapter;
 ...
 */
public class PostStaFragList extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_post_sta_frag_list, container, false);
    }
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            ...
                            if(dataSnapshot.exists()){
                                for(DataSnapshot snapshot: dataSnapshot.getChildren()){
                                    Post post = snapshot.getValue(Post.class);
                                    if(post.getUid().equals(uid)) {
                                        if (post.getModified().startsWith(currentDate)) {
                                            postList.add(post);
                                        }
                                    }
                                }
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            ...
                            if(dataSnapshot.exists()){
                                for(DataSnapshot snapshot: dataSnapshot.getChildren()){
                                    Post post = snapshot.getValue(Post.class);
                                    if(post.getUid().equals(uid)) {
                                        if (post.getModified().contains(currentMonth)) {
                                            postList.add(post);
                                        }
                                    }
                                }
